...
public sealed class TSParser : IDisposable
{
    ...
    public void set_logger(TSLogger logger)
    {
        ...
        var data = new _TSLoggerData {Log = code.LogCallback};
    ...
    */
    ...
    private static extern bool ts_parser_set_included_ranges(IntPtr parser,
        [In, MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 2)] TSRange[] ranges, uint length);
    ...
    */
    ...
    private static extern IntPtr ts_parser_parse_string_encoding(IntPtr parser, IntPtr oldTree,
        [MarshalAs(UnmanagedType.LPWStr)] string input, uint length, TSInputEncoding encoding);
...
public sealed class TSQueryCursor : IDisposable
{
    ...
    [DllImport("tree-sitter.dll", CallingConvention = CallingConvention.Cdecl)]
    private static extern bool ts_query_cursor_next_capture(IntPtr cursor, out TSQueryMatch match,
...
public sealed class TSLanguage : IDisposable
{
    ...
    public TSLanguage(IntPtr ptr)
    {
        ...
        {
            symbols[i] = Marshal.PtrToStringAnsi(ts_language_symbol_name(Ptr, i)) ?? string.Empty;
        }
        ...
        {
            fields[i] = Marshal.PtrToStringAnsi(ts_language_field_name_for_id(Ptr, i)) ?? string.Empty;
